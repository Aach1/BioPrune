...
class GraphTransformerEncoder(nn.TransformerEncoder):
    def forward(
        ...
        update_x=None,
        head_mask: Optional[Union[torch.Tensor, Dict]] = None,
    ):
        output = x
        attn_maps: List[Optional[torch.Tensor]] = [] if return_attn else None
                ...
                if return_attn:
                    ...
                    if head_mask is not None:
                        if isinstance(head_mask, dict):
                            mask_i = head_mask.get(i, None)
                        elif isinstance(head_mask, torch.Tensor):
                            if head_mask.dim() == 2 and head_mask.shape[0] > i:
                                mask_i = head_mask[i]
                    out = mod(
                        output,
                        edge_index,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if max_seq_len is None:
            self.classifier = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(True), nn.Linear(d_model, num_class))
        else:
            self.classifier = nn.ModuleList()
            for i in range(self.max_seq_len):
                self.classifier.append(nn.Linear(d_model, num_class))

    def forward(self, data, return_attn: bool = False, head_mask: Optional[Union[torch.Tensor, Dict]] = None):
        ...
        enc_out = self.encoder(
            ...
            ptr=None,
            return_attn=return_attn,
            update_x=data[3] if len(data) > 3 else None,
            head_mask=head_mask,
        )
        if return_attn:
            output, attn_maps = enc_out
        else:
            output = enc_out
            attn_maps = None
